...
class gmpf
{
	...
	*/
	...
	function admin_menu()
	{
		add_menu_page(__("Google Feeds",'gmpf'), __("Google Feed",'gmpf'), 'manage_options', 'edit.php?post_type=gmpf', false, false, '80.025');
	}
	...
	*/
	...
	function post_updated_messages( $messages )
	{
		...
		$messages['gmpf'] = array(
			...
			1 => __('Feed updated.', 'gmpf'),
			2 => __('Feed updated.', 'gmpf'),
			3 => __('Feed deleted.', 'gmpf'),
			4 => __('Feed updated.', 'gmpf'),
			/* translators: %s: date and time of the revision */
			5 => isset($_GET['revision']) ? sprintf( __('Field group restored to revision from %s', 'gmpf'), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
			6 => __('Feed published.', 'gmpf'),
			7 => __('Feed saved.', 'gmpf'),
			8 => __('Feed submitted.', 'gmpf'),
			9 => __('Feed scheduled for.', 'gmpf'),
			10 => __('FFeed draft updated.', 'gmpf'),
		);
